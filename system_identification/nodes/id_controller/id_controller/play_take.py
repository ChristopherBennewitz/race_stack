...
class TakePlayer(Node):
    ...
    def _begin_figure_eight_lobe(self) -> None:
        """Start a new M3 lobe without moving its fixed reference path."""
        self.feedback = 0.0
    ...
    def _contained_command(self, nominal_steer: float,
        ...
        if self.figure_eight is not None and abs(nominal_steer) > 1e-6:
            # Keep the displayed reference fixed, but compare against only the
            # left- or right-hand lobes matching the current steering direction.
